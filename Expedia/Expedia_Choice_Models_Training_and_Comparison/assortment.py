...
def cal_profict_pro(model,valid_data,price,assortment,feature_column):
    valid_data = valid_data.reshape((2,1,100,12))
    temp_data = valid_data[:,:,assortment,:]
    data_temp = np.zeros((1,len(assortment),6))
    data_cate_temp = np.zeros((1,len(assortment), 4))
    ...
    for j in range(len(assortment)):
        if temp_data[1,0,j,5] == np.min(temp_data[1,0,:,5]):
            data_cate_temp[0,j,0] = 1
        if temp_data[1,0,j,5] == np.max(temp_data[1,0,:,5]):
            data_cate_temp[0,j,1] = 1
        if temp_data[1,0,j,6] == np.min(temp_data[1,0,:,6]):
            data_cate_temp[0,j,2] = 1
        if temp_data[1,0,j,6] == np.max(temp_data[1,0,:,6]):
            data_cate_temp[0,j,3] = 1
    a1 = np.concatenate([temp_data[0],np.zeros((1, len(assortment), 6)),data_cate_temp],axis = 2)
    b1 = np.concatenate([temp_data[1],data_temp,np.ones((1,len(assortment),4))],axis = 2)
...
#
    ...
    profit = 0
    assortment = []
    for k in range(max_assort):
        revenue = {}
        if len(assortment) == capacity:
            return assortment
        for idx in range(max_assort):
            if idx not in assortment:
                temp_assort = assortment + [idx]
                revenue[idx] = cal_profict(model,valid_data,price,temp_assort)
        max_idx  = max(revenue, key = revenue.get)
        max_revunue = revenue[max_idx]
        if profit > max_revunue:
            return assortment
        else:
            profit = max_revunue
            assortment.append(max_idx)
    return assortment

def  ml_assortment_swap(model,valid_data,price,assortment,max_assort = 100,capacity = INFINITY,feature_column = [i for i in range(22)]):
    assortment = list(assortment)
    profit = cal_profict_pro(model,valid_data,price,assortment,feature_column)[0]
    for k in range(10000):
        ...
        for idx,item in enumerate(assortment):
            ...
            revenue[idx] = cal_profict_pro(model,valid_data,price,temp_assort,feature_column)[0]
...
def cal_profict_transform(model_name, df, assortment, pt_weight, price):
    ...
    pro_pred = Modifiled_Transformer.predict(df.loc[assortment],max_num=30,pt_weight = pt_weight,model_name = model_name )
