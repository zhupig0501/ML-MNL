...
class PurchaseTransformerEncoder(d2l.Encoder):
    """Transformer Encoder Block"""
    def __init__(self, key_size, query_size, value_size,
                 num_hiddens, norm_shape, ffn_num_input, ffn_num_hiddens,
                 num_heads, num_layers, dropout, feature_size_list, k, embedding_sizes, norm_shape_init, device, choice_size, use_bias=False, **kwargs):
        super(PurchaseTransformerEncoder, self).__init__(**kwargs)
        ...
        self.dense2 = nn.Linear(choice_size, 1)
        self.sig = nn.Sigmoid()
        self.device = device
    ...
    def forward(self, x, *args):
        ...
        shape0 = x_final.shape[0]
        x_final = x_final.squeeze(-1)
        # mask
        
        shape = x_final.shape[1]
        x_final = self.dense2(x_final)
