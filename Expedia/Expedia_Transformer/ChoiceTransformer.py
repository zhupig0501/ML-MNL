...
from True_Model.Transformer import TransformerEncoder
from True_Model.Transformer import EarlyStopping
import math
from True_Model.Transformer import CosineScheduler
from True_Model.Transformer import MaskedSoftmaxCELoss
...
class ChoiceTransformer():
    def __init__(self, non_purchase_flag=False, weight=30):
        ...
        self.criterion = MaskedSoftmaxCELoss(self.weight).to(self.device)
    ...
    def train_model(self, model_path=None):
        ...
        # self.load_model()
        # # test model
        # self.test_choice_transformer()

    def initialize_model(self, batch_path=None, model_path=None):
        if batch_path is None:
            batch_path = "./True_Model/data/batch_size_ct.npy"
    ...
    def set_hyperparameter_and_model_path(self,batch_size=None, model_path=None):
        ...
        if self.model_path is None:
            np.save(".\\True_Model\\data\\batch_size_ct.npy", self.hp.batch_size)
        else:
            np.save(".\\True_Model\\data\\batch_size_ct_" +self.model_path +  ".npy", self.hp.batch_size)
    ...
    def train_choice_transformer(self):   
        ...
        self.model = TransformerEncoder(
            ...
            self.hp.num_layers, self.hp.dropout, self.feature_size_list, self.hp.k, self.hp.embedding_sizes, self.hp.norm_shape_init, self.device)
        ...
        #         pdb.set_trace()
                ...
                optimizer.step()
            print('Epoch', epoch, 'loss', np.sqrt(loss_total / length_total))
        #         if step % DISPLAY == 0:
        #     scheduler.step()
            with torch.no_grad():
                self.model.eval()
                ...
                for step, batch in enumerate(self.valid_batches):
                    ...
                    j = outputs.shape[0]
                    for jj in range(j):
                        if self.non_purchase_flag:
                            if y[jj][valid_lens[jj]-1] == 1:
                                non_purchase_acc += int(outputs[jj, 0:valid_lens[jj], 0].argmax(dim=-1)) == y[jj].argmax(dim=-1)
                                non_purchase_count += 1
                            else:
                                # normal purchase acc
                                # purchase_acc += int(outputs[jj, 0:valid_lens[jj], 0].argmax(dim=-1)) == y[jj].argmax(dim=-1)
                                # purchase acc without non_purchase
                                purchase_acc += int(outputs[jj, 0:(valid_lens[jj]-1), 0].argmax(dim=-1)) == y[jj].argmax(dim=-1)
                                purchase_count += 1
                        else:
                                acc += int(outputs[jj, 0:valid_lens[jj], 0].argmax(dim=-1)) == y[jj].argmax(dim=-1)
                if self.non_purchase_flag:
                    print("valid loss {:g}, purchase_acc {:g}, non_purchase_acc {:g}".format(np.sqrt(loss_total / length_total), purchase_acc / purchase_count, non_purchase_acc / non_purchase_count))
                    print("acc {:g}".format((purchase_acc + non_purchase_acc) / self.valid_length))
                    print("valid purchase length {:g}, non_purchase length {:g}".format(purchase_count, non_purchase_count))
                else:
                    print("valid loss {:g}, accuracy {:g}".format(np.sqrt(loss_total / length_total), acc / self.valid_length))
    ...
    def generate_data_batch(self):
        ...
        indices = np.random.permutation(self.test_length)
        test_data_conti, test_data_cat, test_label, test_valid_len = self.data_conti, self.data_cat, self.choice_label, self.valid_len
        test_data = [test_data_cat[indices], test_data_conti[indices], test_label[indices],
                    test_valid_len[indices]]
        i = 0
        while i < self.test_length:
    ...
    def load_model(self, model_path=None):
        ...
        if model_path is None:
            ...
            model_path = ".\\True_Model\\checkpoints\\" + model_path + ".pth"
        self.model.load_state_dict(torch.load(model_path,map_location=torch.device('cpu')))
        self.model = self.model.to(self.device)

    def test_choice_transformer(self):
        with torch.no_grad():
            self.model.eval()
            loss_total = 0
            length_total = 0
            ...
            for step, batch in enumerate(self.test_batches):
                ...
                for jj in range(j):
                    if self.non_purchase_flag:
                        ...
                        acc += int(outputs[jj, 0:valid_lens[jj], 0].argmax(dim=-1)) == y[jj].argmax(dim=-1)
            if self.non_purchase_flag:
                print("test loss {:g}, non_purchase_acc {:g}, purchase_acc {:g}".format(np.sqrt(loss_total / length_total), non_purchase_acc / non_purchase_count, purchase_acc / purchase_count))
                print("acc {:g}".format((purchase_acc + non_purchase_acc) / self.test_length))
            else:
                print("test loss {:g}, accuracy {:g}".format(np.sqrt(loss_total / length_total), acc / self.test_length))
...
# choice_tf = choice_transformer()
# # for mode = mean
# choice_tf.load_data()
# # for mode = 0-1
# # choice_tf.load_data('category_feature_sizes_0_and_minus1', 'data_0_and_minus1_cat', 'data_0_and_minus1_conti', 'label_0_and_minus1', 'valid_len_0_and_minus1')
# choice_tf.train_model()
...
# choice_tf.set_hyperparameter_and_model_path(batch_size)
# choice_tf.generate_data_batch()
# choice_tf.load_model(model_path)

# choice_tf.test_model()