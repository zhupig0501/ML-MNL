...
from choicemodels_real_data import MNL_loss,MMNL_loss,Exp_loss,MLE,DeepCell,EarlyStopping,DeepCell_With_Assortment,Exp_loss
...
# Estimate the LC-MNL
def CG_algo(training_data,label,train_dic,BATCH_SIZE = 32,NUM_EPOCHS = 100,LR = 0.005):
    total = len(train_dic)
    total_assort_id_list = list(train_dic)
    iterate_times = 100
    
    #set MNL model as initial solution
    model_list = [torch.load("Assortment\\MNL_parameters_ztn.pt")]
    alpha = np.array([1])
